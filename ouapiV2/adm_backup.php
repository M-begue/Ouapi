...
/**
 * Fonction pour limiter le nombre de sauvegardes
 * Supprime les plus anciennes si le nombre dépasse le maximum
 */
...
if (isset($_GET['deleted']) && $_GET['deleted'] == 1) {
	$template->assign_block_vars('backup_message', array(
		...
		'TEXT' => $lang["backup_deleted_text"] ?? 'La sauvegarde a été supprimée avec succès',
	));
}
...
if (isset($_GET['created']) && $_GET['created'] == 1) {
	$template->assign_block_vars('backup_message', array(
		'TYPE' => 'success',
		'TITLE' => $lang["backup_created_success"] ?? 'Sauvegarde créée',
		'TEXT' => $lang["backup_created_text"] ?? 'La sauvegarde a été créée avec succès',
	));
}
