...
if (isset($_GET['action']) && $_GET['action'] == 'add' && isset($_POST['soumettre']))
{
	...
	//Controle si la date de d�but est inf�rieure � la date de fin
	if ($date_deb > $date_fin)
		array_push($err,$lang["reserv_startenddateerr"]);

	// Controle si les dates de d�but et fin sont valides
...
elseif (isset($_GET['action']) && $_GET['action'] == 'Gerer')
{
	...
	// SITE
	// Si l'utilisateur a les droits d'�dition
	...
	// Si l'utilisateur n'a PAS les droits d'�dition
	else
	{
		...
		if (count($tab_resa) > 0)
		{
			...
			{						
				$template->assign_block_vars('form.histo.list', array(
				  ...
				  'STARTDATE' => date("d/m/Y H:i", strtotime($tab_resa[$i]["date_deb"])),				
				  'ENDDATE' => date("d/m/Y H:i", strtotime($tab_resa[$i]["date_fin"])),				
				));
