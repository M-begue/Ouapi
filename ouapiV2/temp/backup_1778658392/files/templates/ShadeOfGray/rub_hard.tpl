...
<!-- BEGIN r_hard -->
	...
	<!-- BEGIN add -->
	<p class="toolbox">
		<img src="{r_hard.add.IMAGE}" style="vertical-align:middle" border="0" alt="" />&nbsp;
		<a href="{r_hard.add.LINK}" target="_blank">{r_hard.add.TEXT}</a>
	</p>
	<!-- END add -->

	<!-- BEGIN display -->
	<p class="toolbox">
		<img src="{r_hard.display.IMAGE}" style="vertical-align:middle" border="0" alt="" />&nbsp;
		<a href="{r_hard.display.LINK}" target="_blank">{r_hard.display.TEXT}</a>
	</p>
	<!-- END display -->		

	<!-- BEGIN rebus -->
	<p class="toolbox">
		<img src="{r_hard.rebus.IMAGE}" style="vertical-align:middle" border="0" alt="" />
		<a href="{r_hard.rebus.LINK}">{r_hard.rebus.TEXT}</a>
	</p>
	<!-- END rebus -->		
	...
	<!-- BEGIN tab_hard -->
		...
		<form method="post" name="formexport" action="index.php?page=export.php&amp;export=mat&amp;type=excel" target="_blank" style="margin:0px;">
			<input type="hidden" name="nom" value="{r_hard.tab_hard.export.NOM}" />
			<input type="hidden" name="export_data" value="{r_hard.tab_hard.export.DATA}" />
			<p class="toolbox">	
				<img src="{r_hard.tab_hard.export.IMAGE}" style="vertical-align:middle" border="0" alt="" /> 
				<a href="#" onclick="{r_hard.tab_hard.export.LINK}"><u>{r_hard.tab_hard.export.TEXT}</u></a>
			</p>
		</form>
		<!-- END export -->
		...
		<table class="table">				
			...
			<tr class="{r_hard.tab_hard.group.list.CLASS_ROW}" id="{r_hard.tab_hard.group.list.ANCHOR}">
				...
				<td class="{r_hard.tab_hard.group.list.CLASS}">
					<!-- BEGIN tools --> 
					<a href="{r_hard.tab_hard.group.list.tools.LINK}" target="_blank">
					<img src="{r_hard.tab_hard.group.list.tools.IMAGE}" border="0" title="{r_hard.tab_hard.group.list.tools.TITLE}" alt =""/></a> 
					<!-- END tools --> 
					&nbsp;
				</td>
			</tr>
	...
	<!-- BEGIN help -->
	<div class="help">
		<img src="{r_hard.help.IMG_HELP}" style="margin-top:-40px; margin-left:-27px;" title="{r_hard.help.IMG_TITLE}" alt="" />{r_hard.help.GENERAL_HELP}
	</div>	
	<!-- END help -->
