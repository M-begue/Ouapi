<!-- BEGIN select -->
	...
	<div class="textbox"><form name="form" action="{select.ACTION}" method="post" enctype="multipart/form-data" onsubmit="return verifErrors()">
		
		<!-- BEGIN elmt_select -->
		<label>{select.elmt_select.TITLE}</label><select name="elmt_id" id="{select.elmt_select.ID}" onchange="{select.elmt_select.CHANGE}" {select.elmt_select.DISABLED}>
			<!-- BEGIN list -->
		...
		<!-- BEGIN doc_select -->
		<label>{select.doc_select.TITLE}</label><select name="doc_id" id="{select.doc_select.ID}" onchange="{select.doc_select.CHANGE}" {select.doc_select.DISABLED}>
			<!-- BEGIN list -->
		...
		<!-- BEGIN comment -->
		<label>{select.comment.TITLE}</label><textarea name="commentaire" {select.comment.DISABLED}>{select.comment.VALUE}</textarea><br/>
		<!-- END comment -->
...
<!-- BEGIN form -->
	...
	<div class="textbox"><form name="form" action="{form.ACTION}" method="post" enctype="multipart/form-data" onsubmit="return verifErrors()">

		<!-- BEGIN type -->
		<label>{form.type.TITLE}</label><select name="type" id="{form.type.ID}" onchange="{form.type.CHANGE}" {form.type.DISABLED}>
			<!-- BEGIN list -->
		...
		<!-- BEGIN entreprise -->
		<label>{form.entreprise.TITLE}</label><select name="entreprise" id="{form.entreprise.ID}" onchange="{form.entreprise.CHANGE}" {form.entreprise.DISABLED}>
			<!-- BEGIN list -->
		...
		<!-- BEGIN date -->
		<label>{form.date.TITLE}</label><input type="text" name="date" value="{form.date.VALUE}" {form.date.DISABLED} />
			<!-- BEGIN action -->
				<a href="{form.date.action.LINK}"><img src="{form.date.action.IMAGE}" border="0" title="{form.date.action.LIBELLE}" alt="" /></a>
			<!-- END action -->	
		<br/>
		<!-- END date -->
		
		<!-- BEGIN date_archive -->
		<label>{form.date_archive.TITLE}</label><input type="text" name="date_archive" value="{form.date_archive.VALUE}" {form.date_archive.DISABLED} />
			<!-- BEGIN action -->
				<a href="{form.date_archive.action.LINK}"><img src="{form.date_archive.action.IMAGE}" border="0" title="{form.date_archive.action.LIBELLE}" alt="" /></a>
			<!-- END action -->	
		<br/>
		<!-- END date_archive -->
		...
		<!-- BEGIN comment -->
		<label>{form.comment.TITLE}</label><textarea name="commentaire" {form.comment.DISABLED} rows="3" cols="30">{form.comment.VALUE}</textarea><br/>
		<!-- END comment -->
