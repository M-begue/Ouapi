...
<div class="information" style="height:65px;margin-top:0;">
  <img src="{soft_infos.ICON}" alt="" style="float:left;margin-right:20px;" />
  <font style="font-weight:bold;font-size:16px;">{soft_infos.NAME}</font><br/>
  {soft_infos.MARQUE}<br/>
  {soft_infos.VERSION}
</div>
<!-- END soft_infos -->
...
<!-- BEGIN form -->
	...
	<div class="textbox_white">
      ...
      <!-- END source_head -->
      <!-- BEGIN softname -->
      <label>{form.softname.TITLE}</label>
      <input type="text" name="nom" value="{form.softname.VALUE}" id="{form.softname.ID}" onkeyup="{form.softname.KEYUP}" onkeydown="if (document.images['imglink_nom']) { document.images['imglink_nom'].src='images/arrow_useredit.gif' }" form.softname.DISABLED} />
      <!-- BEGIN action -->
      ...
      <select name="marque_id" onchange="if (document.images['imglink_marque']) { document.images['imglink_marque'].src='images/arrow_useredit.gif' }; ">
        <!-- BEGIN list -->
      ...
      <!-- BEGIN vnum -->		
        <label>{form.vnum.TITLE}</label>
        <input type="text" name="{form.vnum.NAME}" value="{form.vnum.VALUE}" {form.vnum.DISABLED} />
      <!-- BEGIN action -->
      ...
      <!-- END vnum -->
			...
			<!-- BEGIN vdate -->
			<label>{form.vdate.TITLE}</label>
			<input name="{form.vdate.NAME}" type="text" id="datepicker" value="{form.vdate.VALUE}" />
			<br/>
			<!-- END vdate -->
      ...
      <!-- BEGIN comment -->
        ...
        <textarea name="commentaire" rows="3" cols="30" {form.comment.DISABLED}>{form.comment.VALUE}</textarea><br/>
      <!-- END comment -->
      ...
      <!-- BEGIN hard_assoc -->		
      <!-- BEGIN list -->
        <label>{form.hard_assoc.list.TITLE}</label>
        <input type="checkbox" name="{form.hard_assoc.list.NAME}" value="{form.hard_assoc.list.VALUE}" {form.hard_assoc.list.CHECKED} /><br/>
      <!-- END list -->
...
<!-- BEGIN form_ocs -->
	...
	<div class="textbox">
		...
		<!-- BEGIN softname -->
			<label>{form_ocs.softname.TITLE}</label>
      <input type="text" name="nom" value="{form_ocs.softname.VALUE}" onKeyUp="verifLong(this);" id="{form_ocs.softname.ID}" {form_ocs.softname.DISABLED} /><br/>
		<!-- END softname -->

		<!-- BEGIN soft_alias -->
		<label>{form_ocs.soft_alias.TITLE}</label>
    <select name="soft_id" id="{form_ocs.soft_alias.ID}" {form_ocs.soft_alias.DISABLED}>
			<!-- BEGIN list -->
				<option value="{form_ocs.soft_alias.list.ID}" {form_ocs.soft_alias.list.SELECTED}>{form_ocs.soft_alias.list.LIBELLE}</option>
			<!-- END list -->
		</select>
		<!-- BEGIN action -->
			<a href="{form_ocs.soft_alias.action.LINK}" target="_blank">
      <img src="{form_ocs.soft_alias.action.IMAGE}" border="0" title="{form_ocs.soft_alias.action.LIBELLE}" alt="" /></a>
		<!-- END action -->	
		<br/>
		<!-- END soft_alias -->
	...
	<!-- BEGIN tab_hard -->	
        ...
        <form name="form" action="index.php?page=adm_logiciels.php&action=MAJ_hard" method="POST" style="padding:0;margin:0" target="_blank">						
          ...
          <input type="submit" name="soumettre" value="{select.tab_hard.list.L_INSTALL}" class="non_form" style="height:16px;font-size:10px;" />
				...
				<td class="row1">
        <a href="{select.tab_hard.list.LINK_DEL}" target="_blank">
				<img src="{select.tab_hard.list.IMG_DEL}" border="0" title="{select.tab_hard.list.TITLE_DEL}" style="vertical-align:middle" alt="" /></a>
        </td>
			</tr>
		<!-- END list -->		
		</table>
