...
<!-- BEGIN form_addlink -->
	...
	<form name="form" action="{form_addlink.ACTION}" method="post" onSubmit="return verifErrors()">
		...
		<div style="float:left;">
			...
			<input type="hidden" name="color" value="2956B2" /><br/>
	
			<!-- BEGIN jscolor -->
				<script type="text/javascript" src="scripts/jscolor.js"></script>
				<label>{form_addlink.L_COLOR}</label><input class="color" value="66ff00" />
			<!-- END jscolor -->
			...
			<label>{form_addlink.L_IMAGE}</label>
			<p class="input" style="border:0;">
			<!-- BEGIN image -->
				<span style="background-color:#CCC;padding:0px;padding-bottom:5px;"><img src="{form_addlink.image.SRC}" alt="" style="width:20px;" onclick="{form_addlink.image.ONCLICK}"/></span>
				<!-- END image -->
			</p>
			<input type="hidden" name="image" value="ouapi.png" />
			...
			<input type="submit" name="soumettre" value="{form_addlink.BUTTON}" />
		</div>
	</form>
	</div>
		
		<div style="float:left;border:0px solid black;margin-left:10px;padding:0px;padding-right:3px;width:98px;height:86px;margin-top:5px; color:#FFF">
			<span style="position:relative;top:0px;background-color:#444;border:1px solid #999;padding-left:5px;padding-right:5px;font-weight:bold;border-radius: 2px 2px; -moz-border-radius: 2px; -webkit-border-radius: 2px;";>{form_addlink.L_PREVIEW}</span>
			<span id="preview" class="my_button" style="{form_addlink.PREVIEW_STYLE}">
				<span id="button_text"></span>
				<img id="preview_image" src="images/gallery/blank.png" border="0" alt="" style="position:absolute;bottom:10px;left:10px;"/>
			</span>		
		</div>
