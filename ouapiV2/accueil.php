	...
	if (isset($_GET["rubrique"]))
	{
		...
		if ($_GET["rubrique"] == "hard")
		{
			...
			$requete = "SELECT ".TAB_HARD.".*,
			  ...
			  WHERE ".TAB_HARD.".".HA_SITEID."='".intval(intval($_GET["agence_id"]))."'".$suivi_rebus."".$filtre_type_id."
			...
			if (count($tab) > 0)
			{
				...
				while ($i < count($tab))
				{
					...
					if ($i == 0 || (trim($cols_groupcol) != '' && $valeur_actuelle != $valeur_precedente))
        			{
            			...
            			if (trim($cols_groupcol) != '') {
    						...
    						if (strpos($cols_groupcol, 'type') !== false) {
        						...
        						// Pour Nom, IP, Numéro de série...
        						// On récupère ce qui est après le dernier point (ex: 'nom' dans 'ouapi_hardware.nom')
					...
					while ($j < count($cols_display))
					{
            			...
            			} elseif (substr($col_brute, 0, 4) == "OCS_") {
                			$valeur_brute = ($cle !== FALSE) ? $tab_ocs[substr($col_brute, 4)][$cle] : '';
            			} else {
			...
			if (count($tab) > 0)
			{
				$serialized_string = htmlentities(serialize($export_data));
		...
		elseif ($_GET["rubrique"] == "docs")
		{
			...
			if (isset($_GET["archive"]) && $_GET["archive"] == "1")
				...
				$suivi_archive = " AND (".TAB_DOCS.".".DO_DATEARCHIVE."='' OR ".TAB_DOCS.".".DO_DATEARCHIVE.">UNIX_TIMESTAMP())";
