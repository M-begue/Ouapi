...
<div class="textbox">
	...
	<!-- BEGIN serial -->
	<label>{form.serial.TITLE}</label><input type="text" name="num_serie" value="{form.serial.VALUE}" onkeydown="if (document.images['imglink_serial']) { document.images['imglink_serial'].src='images/arrow_useredit.gif' }" {form.serial.DISABLED} />
		<!-- BEGIN action -->
	...
	<!-- BEGIN marque -->
	<label>{form.marque.TITLE}</label><select name="marque" onchange="{form.marque.ONCLICK}">
		<!-- BEGIN list -->
	...
	<!-- BEGIN modele -->
	<label>{form.modele.TITLE}</label><span id="bloc_periph_modele">
	<select name="modele" onchange="if (document.images['imglink_modele']) { document.images['imglink_modele'].src='{form.TEMPLATE_ROOT}/arrow_useredit.gif' }">
		...
		<!-- BEGIN action -->
			<a href="{form.modele.action.LINK}" id="link_periph_modele"><img src="{form.modele.action.IMAGE}" name="imglink_modele" border="0" title="{form.modele.action.LIBELLE}" alt="" /></a>
		<!-- END action -->	
		<!-- BEGIN valid -->
	...
	<!-- BEGIN linkhard -->
	<label>{form.linkhard.TITLE}</label><select name="hard" onchange="if (document.images['imglink_linkhard']) { document.images['imglink_linkhard'].src='{form.TEMPLATE_ROOT}/arrow_useredit.gif' }">
		<!-- BEGIN list -->
	...
	<!-- BEGIN comment -->
	<label>{form.comment.TITLE}</label><textarea name="commentaire"  rows="3" cols="50" onkeydown="if (document.images['imglink_comment']) { document.images['imglink_comment'].src='{form.TEMPLATE_ROOT}/arrow_useredit.gif' }">{form.comment.VALUE}</textarea>
		<!-- BEGIN valid -->
	...
	<!-- BEGIN suivi_rebus -->
	<label>{form.suivi_rebus.TITLE}</label><textarea name="suivi" rows="10" cols="50" style="vertical-align:top">{form.suivi_rebus.VALUE}</textarea><br/>
	<!-- END suivi_rebus -->
	...
	<!-- BEGIN key -->	
		...
		<img src="{form.key.TEMPLATE_DIR}images/arrow_forbid.gif" style="vertical-align:middle;margin-left:10px;" alt="" />&nbsp;{form.key.L_FORBID}</td>
		</div>
	<!-- END key -->	
...
<div class="textbox">
	...
	<!-- BEGIN periph -->
	<label>{addlink.periph.TITLE}</label><select name="periph_id" id="large">
		<!-- BEGIN list -->
