...
<!-- BEGIN select -->
    ...
    <tr>
			...
			<td class="row1">
				<!-- BEGIN option -->
					{select.records.list.connex.option.LIBELLE}&nbsp;
          <input type="checkbox" name="{select.records.list.connex.option.NAME}" value="{select.records.list.connex.option.VALUE}" class="non_form" onclick="submit()" {select.records.list.connex.option.CHECKED} style="border:0" />&nbsp;&nbsp;
				<!-- END option -->
			</td>
			<!-- END connex -->
			...
			<!-- BEGIN place -->
				<td class="row1">
        <select name="site_id" class="non_form" onchange="submit()">
				<!-- BEGIN option -->
			...
			<!-- BEGIN marque -->
				<td class="row1">
        <select name="marque_id" class="non_form" onchange="submit()">
          <!-- BEGIN option -->
	...
	<!-- BEGIN post_result -->
      ...
      <div class="{select.post_result.CLASS}"><img src="{select.post_result.ICON}" alt="" style="vertical-align:middle;margin-right:5px;" />{select.post_result.MESSAGE}</div>
      </td>
		</tr>
		</table>
	<!-- END post_result -->
...
<!-- BEGIN form -->
	...
	<div class="textbox_white">
      ...
      <input type="text" name="libelle" value="{form.libelle.VALUE}" id="{form.libelle.ID}" onKeyUp="{form.libelle.KEYUP}" {form.libelle.DISABLED} />		
			<br/>
      <!-- END libelle -->
  
      <!-- BEGIN multisite -->
      <label>{form.multisite.TITLE}</label>
      <select name="site_id" {form.multisite.DISABLED}>
        <!-- BEGIN list -->
      ...
      <select name="marque_id" {form.marque.DISABLED}>
        <!-- BEGIN list -->
          <option value="{form.marque.list.ID}" {form.marque.list.SELECTED}>{form.marque.list.LIBELLE}</option>
        <!-- END list -->
      </select><br/>
			<label>{form.marque.IMAGE}</label>
        <!-- BEGIN image-->
          <input type="file" name="doc" value="{form.marque.VALUE}" id="large" {form.marque.DISABLED} /><br/>
        <!-- END image-->
      <!-- END marque -->

      <!-- BEGIN connex -->		
        <label>{form.connex.TITLE}</label>
        <p class="input" style="border:0px;margin-bottom:0;">
        <!-- BEGIN list -->
          {form.connex.list.LIBELLE}&nbsp;
          <input type="checkbox" name="{form.connex.list.NAME}" value="{form.connex.list.VALUE}" class="non_form" {form.connex.list.CHECKED} />&nbsp;&nbsp;
        <!-- END list -->
        </p>
      <!-- END connex -->
...
<!-- BEGIN form_prepost -->
	...
	<tr>
		<td class="warning">
    <form name="form" action="{form_prepost.ACTIONLABEL}" method="post" >
		<!-- BEGIN warning -->
		...
		<!-- BEGIN list_field -->
		<label>{form_prepost.list_field.TITLE}</label>
    <select name="{form_prepost.list_field.NAME}" {form_prepost.list_field.DISABLED}>
			<!-- BEGIN list -->
