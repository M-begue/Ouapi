...
/*		AJOUT DE LA FONCTIONNALITE D4IMAGE DANS models of system
		...
		// Si la table est li�e a un site
		if (isset($_POST['site_id']))
		{
			if ($_POST["site_id"] == 'all')
			{
				...
				// Si le libelle n'existe pas d�j�
		...
		else
		{
			...
			if (count($tab_verif) == 0)
			{			
				// Cas particulier de l'ajout de type de mat�riel
	...
	else
	{
		...
		// Cas particulier des tables li�e � une agence
		...
		// Cas particulier de la table des type de mat�riel >> Connexions possibles
		...
		// Cas particulier des tables modeles li�es � une marque
...
else
{
	...
	if (isset($_GET['action']) && $_GET['action'] == 'delete')
	{
		if (isset($_GET['table']) && $_GET['table'] == 'agences')
		{
			...
			else
			{
				...
				if ($siteid != 1)
				{
					...
					// Tout est supprim�, le site peut �tre supprim� (sauf si site 1)
					...
					// Tout n'est pas supprim�, on propose un site de remplacement
		...
		else
		{
			...
			$post_result = str_ireplace('L\'�l�ment', strtoupper($tab_element[0]["libelle"]), $lang["adm_table_delok"]);
			$post_class = 'table_success';
			$post_icon = 'templates/'.DEFAULT_TEMPLATE.'/images/ok.png';
		}
	}
	...
	if (isset($_POST['table']))
	{
		...
		if (isset($_POST['site_id']))
		{
			...
			{	
				...
				$post_result = str_ireplace('L\'�l�ment', strtoupper($libelle), $lang["adm_table_editok"]);
				$post_class = 'table_success';
				$post_icon = 'templates/'.DEFAULT_TEMPLATE.'/images/ok.png';
			}
			else
		...
		if (count($err) > 0)
		{
			$post_result = str_ireplace('L\'�l�ment', strtoupper($libelle), $lang["adm_table_editerror"]);
	...
	if (count($tab) > 0)
	{
		...
		/****************** DEBUT Gestion des colonnes particuli�res ***********************/
		if ($_GET["table"] == "hard_type")
		{
		...
		/******************** FIN Gestion des colonnes particuli�res ***********************/
